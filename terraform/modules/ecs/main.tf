...
resource "aws_alb" "alb_ovassistant" {
  name            = "${var.environment}-alb-ovassistant"
...
resource "aws_alb_listener" "ovassistant" {
  load_balancer_arn = "${aws_alb.alb_ovassistant.arn}"
  port              = "80"
  protocol          = "HTTP"
