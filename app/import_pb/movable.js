...
/**
 * Returns the speed per second from ‘this’ point to destination point (using haversine formula).
 *
 * @param   {LatLon} point - Latitude/longitude of destination point.
 * @param   {prevDatetime} int - Unix timestamp of 'this' point. 
 * @param   {nextDatetime} int - Unix timestamp of destination point. 
 * @param   {number} [radius=6371e3] - (Mean) radius of earth (defaults to radius in metres).
 * @returns {number} Speed per second in meters between this point and destination point.
 *
 * @example
 *     var p1 = new LatLon(52.205, 0.119);
 *     var p2 = new LatLon(48.857, 2.351);
 *     var prevDatetime = 1543145907;
 *     var nextDatetime = 1543145937;
 *     var speed = p1.distanceTo(p2, prevDatetime, nextDatetime); // 5.477 meters
 */
LatLon.prototype.speedPerSecond = function(point, prevDatetime, nextDatetime, radius) {
    if (!(point instanceof LatLon)) throw new TypeError('point is not LatLon object');
    radius = (radius === undefined) ? 6371e3 : Number(radius);
    ...
    var timeDiff = nextDatetime - prevDatetime 
    var speedPerSecond = d / (timeDiff > 0 ? timeDiff : 1)
