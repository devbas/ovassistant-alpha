...
const getVehicleCandidates = async (data) => { 
  ...
  try {
    ...
    const vehicleCandidatesRaw = await axios.get(`http://nearest:9001/classify/location?lon=${data.lon}&lat=${data.lat}&datetime=${data.datetime}&user_id=${data.userId}&prev_user_location=${prevUserLocation[0] ? JSON.stringify(prevUserLocation[0]) : ''}`)                                  
