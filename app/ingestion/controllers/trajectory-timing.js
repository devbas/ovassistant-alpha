...
const updateTrajectoryTiming = async ({ vehicleId, delaySeconds, measurementUnix, pgPool }) => {
  ...
  try {
    ...
    if(trip[0].trip_id) {
      await client.query(`UPDATE trip_times_partitioned AS TTP1
                            SET 
                            TTP1.start_planned = TTP2.start_planned + TTP2.delay_seconds - $1, 
                            TTP1.end_planned = TTP2.end_planned + TTP2.delay_seconds - $2, 
                            TTP1.delay_seconds = $3
