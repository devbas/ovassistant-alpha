...
router.post('/user-location', (req, res) => {
  const data = req.body; 
  matchCalculationRedisController.userLocationMatch(data, function(err, match) {
    if(err) {
...
router.get('/info/:vehicleType/:vehicleId/:timetableDatetime', async (req, res) => {
  // const data = req.query; 
  console.log('data: ', req.params.vehicleId)
  try {
    // const info = await matchCalculationController.parseVehicleItemInfo({ vehicleId: req.params.vehicleId, type: req.params.vehicleType })
    const datetime = moment.unix(req.params.timetableDatetime).tz('Europe/Amsterdam').format('YYYY-MM-DD HH:mm:ss')
    const info = await matchCalculationController.getVehicleContext({ vehicleId: req.params.vehicleId, type: req.params.vehicleType, datetime: datetime })    
